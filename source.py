...
if os.path.exists("/data/data/com.termux/files/home/ScoRe"):
    os.chdir(os.path.expanduser("~"))
    result = subprocess.run(["cp", "-r", "/data/data/com.termux/files/home/ScoRe", "."], capture_output=True, text=True)
    if result.returncode == 0:
        print("✎┊‌ Copied ScoRe directory successfully. (✓✓)")
        time.sleep(10)  
        if os.path.exists(os.path.expanduser("~/ScoRe")):
            os.chdir(os.path.expanduser("~/ScoRe"))
            ...
            print("✎┊‌ Failed to access ScoRe directory after 5 seconds delay. (❌❌❌)")
    else:
        print(f"✎┊‌ Failed to copy ScoRe directory: {result.stderr} (❌❌❌)")
else:
    print("✎┊‌ Error: ScoRe directory not found in Termux home. (❌❌❌)")
